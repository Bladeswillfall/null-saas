...
import {
  ingestSourceRecordsForBatch,
  matchSourceRecordsForBatch,
...
type ImportBatchAutoReviewStatus =
  | "pending"
  | "ready"
  | "needs_manual_review"
  | "published";

type ImportBatchAutoReviewSummary = NonNullable<
  ImportBatchSummary["autoReviewSummary"]
>;
...
async function withAnalyticsQuery<T>(
  fallback: T,
  action: () => Promise<T>,
...
function sortByNewest<T>(
  ...
  return [...rows].sort(
    (left, right) => asTimestamp(accessor(right)) - asTimestamp(accessor(left)),
  );
}

function selectBestProvenance(
  tags: AnalyticsProvenanceTag[],
): AnalyticsProvenanceTag {
  const order: AnalyticsProvenanceTag[] = [
    "direct",
    "estimated",
    "engagement",
    "awards",
...
function mapIp(
  row: typeof franchises.$inferSelect,
  workCount: number,
): AnalyticsIp {
  return {
    id: row.id,
...
function normalizeAutoReviewSummary(
  value: unknown,
): ImportBatchAutoReviewSummary | null {
  ...
  return {
    invalidRowCount: numberValue(summary.invalidRowCount),
    sourceRecordCount: numberValue(summary.sourceRecordCount),
    matchedCount: numberValue(summary.matchedCount),
    needsReviewCount: numberValue(summary.needsReviewCount),
    normalizedCount: numberValue(summary.normalizedCount),
    unresolvedCount: numberValue(summary.unresolvedCount),
    flagCount: numberValue(summary.flagCount),
  };
}
...
async function getOrganizationAnalyticsBase(
  ...
  const [ipRows, workRows] = await Promise.all([
    ctx.db
      .select()
      .from(franchises)
      .where(eq(franchises.organizationId, organizationId)),
...
async function getWorkOrganizationId(
  ctx: AnalyticsContext,
  workId: string,
): Promise<string> {
...
async function getIpOrganizationId(
  ctx: AnalyticsContext,
  ipId: string,
): Promise<string> {
  const [row] = await ctx.db
...
export async function listAnalyticsIps(
  ...
  return withAnalyticsQuery([], async () => {
    const { ipRows, workRows } = await getOrganizationAnalyticsBase(
      ctx,
    ...
    workRows.forEach((row) => {
      if (row.franchiseId) {
        workCountByIp.set(
          row.franchiseId,
          (workCountByIp.get(row.franchiseId) ?? 0) + 1,
        );
      }
    });
...
export async function getAnalyticsIp(
  ...
  return withAnalyticsQuery<AnalyticsIp | null>(null, async () => {
    const [row] = await ctx.db
      ...
      .where(
        and(
          eq(franchises.id, input.ipId),
          eq(franchises.organizationId, input.organizationId),
        ),
      )
      .limit(1);
    ...
    const ipWorks = await ctx.db
      .select({ id: works.id })
      .from(works)
      .where(eq(works.franchiseId, row.id));
  ...
  return withAnalyticsQuery([], async () => {
    ...
    const externalRows = workIds.length
      ? await ctx.db
          ...
          .where(inArray(workExternalIds.workId, workIds))
      : [];

    const externalCountByWork = new Map<string, number>();
    externalRows.forEach((row) => {
      externalCountByWork.set(
        row.workId,
        (externalCountByWork.get(row.workId) ?? 0) + 1,
      );
    });
    ...
    return workRows
      ...
      .filter((row) => {
        ...
        const ipName = row.franchiseId
          ? (ipById.get(row.franchiseId)?.name ?? "")
          : "";
        return canonicalizeTitle(
          `${row.title} ${row.canonicalTitle ?? ""} ${ipName}`,
        ).includes(normalizedQuery);
      })
      ...
      .map((row) =>
        mapWork(
          row,
          row.franchiseId ? (ipById.get(row.franchiseId)?.name ?? null) : null,
          externalCountByWork.get(row.id) ?? 0,
        ),
      );
  });
}
...
export async function getWork(
  ...
  AnalyticsQueryResult<
    (AnalyticsWork & { externalIds: WorkExternalIdRecord[] }) | null
  >
> {
  ...
  >(null, async () => {
    ...
    const [ipRows, providerRows, externalRows] = await Promise.all([
      workRow.franchiseId
        ? ctx.db
            .select()
            .from(franchises)
            .where(eq(franchises.id, workRow.franchiseId))
            .limit(1)
        : Promise.resolve([]),
      ...
      ctx.db
        .select()
        .from(workExternalIds)
        .where(eq(workExternalIds.workId, input.workId)),
    ]);
    ...
    return {
      ...
      externalIds: externalRows.map((row) =>
        mapExternalId(row, providerById.get(row.sourceProviderId)),
      ),
    };
  });
}
  ...
  try {
    ...
    if (row.franchiseId) {
      const [ipRow] = await ctx.db
        .select()
        .from(franchises)
        .where(eq(franchises.id, row.franchiseId))
        .limit(1);
...
export async function updateWork(
  ...
  try {
    ...
    const externalRows = await ctx.db
      .select()
      .from(workExternalIds)
      .where(eq(workExternalIds.workId, row.id));
    let ipName: string | null = null;
    if (row.franchiseId) {
      const [ipRow] = await ctx.db
        .select()
...
export async function listSourceProviders(
  ...
  return withAnalyticsQuery([], async () => {
    ...
    const batchMap = new Map<
      string,
      Array<typeof importBatches.$inferSelect>
    >();
    ...
    return providerRows
      ...
      .sort((left, right) => left.name.localeCompare(right.name))
      .map((row) => {
        const batches = sortByNewest(
          batchMap.get(row.id) ?? [],
          (entry) => entry.completedAt ?? entry.createdAt,
        );
        return mapSourceProvider(
          row,
          batches.length,
          batches[0]?.completedAt ?? batches[0]?.createdAt ?? null,
        );
      });
  });
}
...
function rawObservationToCsvShape(
  record: typeof rawObservations.$inferSelect,
  ...
  return {
    ...
    engagement_count:
      record.engagementCount === null ? "" : String(record.engagementCount),
    sales_value: record.salesValue === null ? "" : String(record.salesValue),
    sales_is_estimated:
      record.salesIsEstimated === null ? "" : String(record.salesIsEstimated),
...
function combineAwardValue(row: Record<string, string>): string | null {
  const parts = [row.award_name?.trim(), row.award_result?.trim()].filter(
    Boolean,
  );
  return parts.length > 0 ? parts.join(": ") : null;
}

async function getBatchOrganizationId(
  ctx: AnalyticsContext,
  batchId: string,
): Promise<string> {
  const [row] = await ctx.db
...
async function getRawObservationOrganizationId(
  ctx: AnalyticsContext,
  ...
  if (!row) {
    throw new TRPCError({
      code: "NOT_FOUND",
      message: "Raw observation not found.",
    });
  }
...
async function getFlagOrganizationId(
  ctx: AnalyticsContext,
  flagId: string,
): Promise<string> {
  const [row] = await ctx.db
...
async function getImportBatchSummaries(
  ctx: AnalyticsContext,
  organizationId: string,
  ...
  const providerById = new Map(
    batchProviders.map((provider) => [provider.id, provider]),
  );
  ...
  const sourceRecordRows = await ctx.db
    .select({
      id: sourceRecords.id,
      importBatchId: sourceRecords.importBatchId,
      ingestionStatus: sourceRecords.ingestionStatus,
    })
    .from(sourceRecords)
    .where(inArray(sourceRecords.importBatchId, batchIds));

  const sourceRecordCountByBatch = new Map<string, number>();
  const reviewQueueCountByBatch = new Map<string, number>();
  sourceRecordRows.forEach((row) => {
    if (!row.importBatchId) {
      return;
    }
    sourceRecordCountByBatch.set(
      row.importBatchId,
      (sourceRecordCountByBatch.get(row.importBatchId) ?? 0) + 1,
    );
    if (row.ingestionStatus === "needs_review") {
      reviewQueueCountByBatch.set(
        row.importBatchId,
        (reviewQueueCountByBatch.get(row.importBatchId) ?? 0) + 1,
      );
    }
  });
  ...
  if (rawIds.length === 0) {
    return batches.map((batch) => ({
      ...
      sourceProviderName:
        providerById.get(batch.sourceProviderId)?.name ?? "Unknown source",
      sourceProviderSlug:
        providerById.get(batch.sourceProviderId)?.slug ?? "unknown",
      importType: batch.importType,
      status: batch.status,
      autoReviewStatus:
        (batch.autoReviewStatus as ImportBatchAutoReviewStatus) ?? "pending",
      rowCount: batch.rowCount,
      errorCount: batch.errorCount,
      normalizedCount: 0,
      unresolvedFlagCount: 0,
      sourceRecordCount: sourceRecordCountByBatch.get(batch.id) ?? 0,
      reviewQueueCount: reviewQueueCountByBatch.get(batch.id) ?? 0,
      publishedAt: batch.publishedAt,
      reviewedAt: batch.reviewedAt,
      reviewedBy: batch.reviewedBy,
      autoReviewSummary: normalizeAutoReviewSummary(batch.autoReviewSummary),
  ...
  const [normalizedRows, unresolvedRows] = await Promise.all([
    ...
    ctx.db
      ...
      .where(
        and(
          inArray(qualityFlags.rawObservationId, rawIds),
          isNull(qualityFlags.resolvedAt),
        ),
      ),
  ]);
  ...
  normalizedRows.forEach((row) => {
    ...
    if (batchId) {
      normalizedCountByBatch.set(
        batchId,
        (normalizedCountByBatch.get(batchId) ?? 0) + 1,
      );
    }
  });
  ...
  unresolvedRows.forEach((row) => {
    if (row.rawObservationId) {
      const batchId = rawToBatch.get(row.rawObservationId);
      if (batchId) {
        unresolvedCountByBatch.set(
          batchId,
          (unresolvedCountByBatch.get(batchId) ?? 0) + 1,
        );
      }
    }
  });
...
function matchRawObservationToWork(
  ...
  if (externalId) {
    const directMatch = externalMap.get(
      `${raw.sourceProviderId}:${externalId}`,
    );
    if (directMatch) {
      return {
        work: directMatch,
  ...
  const rawMediaType = String(
    metadata.media_type ?? raw.rawCategory ?? "",
  ).trim();
  ...
  workRows.forEach((work) => {
    ...
    if (
      rawIpName &&
      work.franchiseId &&
      canonicalizeTitle(ipById.get(work.franchiseId)?.name ?? "") === rawIpName
    ) {
      score += 2;
    }
...
export async function listImportBatches(
  ...
  await requireOrganizationMember(ctx, input.organizationId);
  return withAnalyticsQuery([], async () =>
    getImportBatchSummaries(ctx, input.organizationId),
  );
}

export async function autoReviewStagedImportBatch(
  ctx: AnalyticsContext,
  input: {
    organizationId: string;
    batchId: string;
  ...
  autoReviewStatus: ImportBatchAutoReviewStatus;
  autoReviewSummary: ImportBatchAutoReviewSummary;
  published: boolean;
}> {
  await requireOrganizationAdmin(ctx, input.organizationId);

  try {
    await ingestSourceRecordsForBatch(ctx, input.batchId, input.rows);
    await matchSourceRecordsForBatch(ctx, {
      batchId: input.batchId,
      selectedBy: ctx.user?.id ?? null,
    });
    const normalization = await normalizeImportBatch(ctx, {
      batchId: input.batchId,
    });
    const batchSourceRecords = await ctx.db
      .select({
        id: sourceRecords.id,
        ingestionStatus: sourceRecords.ingestionStatus,
      })
      .from(sourceRecords)
      .where(eq(sourceRecords.importBatchId, input.batchId));

    const needsReviewCount = batchSourceRecords.filter(
      (record) => record.ingestionStatus === "needs_review",
    ).length;
    const matchedCount = batchSourceRecords.filter(
      (record) => record.ingestionStatus === "matched",
    ).length;

    const autoReviewSummary: ImportBatchAutoReviewSummary = {
      invalidRowCount: input.invalidRowCount,
      sourceRecordCount: batchSourceRecords.length,
      matchedCount,
      needsReviewCount,
      normalizedCount: normalization.normalizedCount,
      unresolvedCount: normalization.unresolvedCount,
      flagCount: normalization.flagCount,
    };

    const shouldRequireManualReview =
      input.invalidRowCount > 0 ||
      needsReviewCount > 0 ||
      normalization.unresolvedCount > 0 ||
      normalization.flagCount > 0;

    let autoReviewStatus: ImportBatchAutoReviewStatus =
      shouldRequireManualReview ? "needs_manual_review" : "ready";
    let published = false;

    if (!shouldRequireManualReview && input.autoPublish !== false) {
      await rebuildScores(ctx, { organizationId: input.organizationId });
      autoReviewStatus = "published";
      published = true;
    }
...
export async function publishImportBatch(
...
): Promise<{ success: true; autoReviewStatus: ImportBatchAutoReviewStatus }> {
  ...
  try {
    ...
    return { success: true, autoReviewStatus: "published" };
  } catch (error) {
    ensureAnalyticsMutationAvailable(error);
  }
}
...
export async function uploadCsvImport(
  ...
  try {
    ...
    parsedRows.forEach((row) => {
      ...
      if (
        providerValue &&
        ![provider.slug.toLowerCase(), provider.name.toLowerCase()].includes(
          providerValue,
        )
      ) {
        rowErrors.push({
    ...
    if (validRows.length > 0) {
      await ctx.db.insert(rawObservations).values(
        validRows.map((row) => {
          ...
          return {
            ...
            rawAuthorOrCreator:
              row.values.author?.trim() || row.values.creator?.trim() || null,
            ...
            ratingValue:
              coerceNumber(row.values.rating_value)?.toString() ?? null,
            ...
            salesValue:
              coerceNumber(row.values.sales_value)?.toString() ?? null,
      ...
      await ingestSourceRecordsForBatch(
        ctx,
        batch.id,
        validRows.map((row) => ({
          ...row.values,
          ...
          rating_value:
            row.values.rating_value ?? row.values.rating_avg_text ?? null,
          review_count:
            row.values.review_count ?? row.values.ratings_count_text ?? null,
          rank_value:
            row.values.rank_value ?? row.values.bestseller_rank_raw ?? null,
...
export async function normalizeImportBatch(
  ...
  try {
    ...
    const rawRows = await ctx.db
      .select()
      .from(rawObservations)
      .where(eq(rawObservations.importBatchId, batch.id));
    if (rawRows.length === 0) {
      return {
        normalizedCount: 0,
        unresolvedCount: 0,
    ...
    const existingNormalized = workIds.length
      ? await ctx.db
          .select()
          .from(normalizedObservations)
          .where(inArray(normalizedObservations.workId, workIds))
      : [];
    ...
    const normalizedPayloads: Array<
      typeof normalizedObservations.$inferInsert
    > = [];
    ...
    rawRows.forEach((raw) => {
      const metadata = ((raw.metadataJson ?? {}) as RawMetadata) ?? {};
      const externalId = String(metadata.external_id ?? "").trim();
      const match = matchRawObservationToWork(
        raw,
        workRows,
        ipRows,
        externalRows,
      ...
      if (!externalId) {
        flagPayloads.push({
          ...
          severity: "warning",
          notes:
            "Row is missing an external ID; exact matching is unavailable.",
        });
      }
      ...
      if (match.confidence < 0.85) {
        ...
        flagPayloads.push({
          ...
          severity: "warning",
          notes:
            "Observation matched with low confidence and should be reviewed.",
        });
      }

      const reviewMetric = metrics.find(
        (metric) => metric.metricType === "review_count",
      );
      const ratingMetric = metrics.find(
        (metric) => metric.metricType === "rating_average",
      );
      if (
        ratingMetric &&
        reviewMetric &&
        reviewMetric.metricValue < lowReviewSampleThreshold
      ) {
        flagPayloads.push({
          rawObservationId: raw.id,
          workId: match.work.id,
      ...
      metrics.forEach((metric) => {
        ...
        const priorHistory = sortByNewest(
          historyByMetric.get(historyKey) ?? [],
        ...
        if (metric.metricType === "rank_position" && priorHistory.length > 0) {
          ...
          if (
            Math.abs(previous.value - metric.metricValue) > rankJumpThreshold
          ) {
            flagPayloads.push({
              rawObservationId: raw.id,
              workId: match.work!.id,
        ...
        if (metric.metricType !== "rank_position" && priorHistory.length >= 3) {
          const recentMedian = median(
            priorHistory.slice(0, 5).map((entry) => entry.value),
          );
          if (
            recentMedian > 0 &&
            metric.metricValue > recentMedian * spikeMultiplierThreshold
          ) {
            flagPayloads.push({
              rawObservationId: raw.id,
              workId: match.work!.id,
...
export async function listQualityFlags(
  ...
  return withAnalyticsQuery([], async () => {
    ...
    const batchIds = input.batchId
      ? batchRows.filter((row) => row.id === input.batchId).map((row) => row.id)
    ...
    const allFlags = await ctx.db
      ...
      .where(or(...filters));
    const providerIds = [
      ...new Set(rawRows.map((row) => row.sourceProviderId)),
    ];
    ...
    return sortByNewest(
      allFlags.filter((row) =>
        input.unresolvedOnly ? row.resolvedAt === null : true,
      ),
      (row) => row.createdAt,
    ).map((row) => {
      const raw = row.rawObservationId
        ? rawById.get(row.rawObservationId)
        : undefined;
...
export async function assignRawObservationToWork(
  ...
  const [rawRows, workRows] = await Promise.all([
    ctx.db
      .select()
      .from(rawObservations)
      .where(eq(rawObservations.id, input.rawObservationId))
      .limit(1),
  ...
  if (!raw || !work) {
    throw new TRPCError({
      code: "NOT_FOUND",
      message: "Raw observation or work not found.",
    });
  }
  ...
  try {
    await ctx.db
      .delete(normalizedObservations)
      .where(eq(normalizedObservations.rawObservationId, raw.id));
    const metrics = buildImportedMetrics(rawObservationToCsvShape(raw));
    ...
    if (externalId) {
      const [existing] = await ctx.db
        ...
        .where(
          and(
            eq(workExternalIds.sourceProviderId, raw.sourceProviderId),
            eq(workExternalIds.externalId, externalId),
          ),
        )
        .limit(1);
    ...
    await ctx.db
      ...
      .where(
        and(
          eq(qualityFlags.rawObservationId, raw.id),
          isNull(qualityFlags.resolvedAt),
        ),
      );
...
function buildMetricObservations(
  rows: Array<typeof normalizedObservations.$inferSelect>,
): MetricObservation[] {
  return rows.map((row) => ({
...
function groupNormalizedByWork(
  rows: Array<typeof normalizedObservations.$inferSelect>,
) {
  const grouped = new Map<
    string,
    Array<typeof normalizedObservations.$inferSelect>
  >();
  rows.forEach((row) => {
...
async function getLatestWorkScoreDate(
  ...
  const [row] = await ctx.db
    ...
    .where(
      and(
        inArray(workScores.workId, workIds),
        eq(workScores.timeWindow, window),
      ),
    )
...
async function getLatestIpScoreDate(
  ...
  const [row] = await ctx.db
    ...
    .where(
      and(
        inArray(ipScores.franchiseId, ipIds),
        eq(ipScores.timeWindow, window),
      ),
    )
  ...
  try {
    ...
    if (workIds.length > 0) {
      await ctx.db
        ...
        .where(inArray(scoreComponents.workId, workIds));
      await ctx.db
        .delete(workScores)
        .where(inArray(workScores.workId, workIds));
    }
    if (ipIds.length > 0) {
    ...
    await ctx.db
      ...
      .where(eq(leaderboardSnapshots.snapshotDate, scoreDate));
    ...
    for (const window of analyticsTimeWindows) {
      ...
      const previousScoreMap = await getPreviousWorkScores(
        ctx,
        workIds,
        window,
        scoreDate,
      ...
      workRows.forEach((work) => {
        const filteredRows = (normalizedByWork.get(work.id) ?? []).filter(
          (row) => !start || asTimestamp(row.observedAt) >= start.getTime(),
        );
        ...
        const confidenceScore = scoreConfidence(
          observations,
          workToProviders.get(work.id)?.size ?? 0,
        );
          ...
          componentPayloads.push({
            ...
            provenanceSummary:
              componentType === "ranking"
                ? "rank positions capped at top 250"
                : null,
          });
        });
      });

      const rankedOverall = [...scoreRowsForWindow].sort(
        (left, right) => right.compositeScore - left.compositeScore,
      );
      const rankedByCategory = new Map<
        AnalyticsMediaType,
        typeof scoreRowsForWindow
      >();
      ...
      rankedOverall.forEach((row, index) => {
        ...
        const rankInCategory =
          categoryRows.findIndex(
            (candidate) => candidate.workId === row.workId,
          ) + 1;
    ...
    for (const window of analyticsTimeWindows) {
      const previousIpMap = await getPreviousIpScores(
        ctx,
        ipIds,
        window,
        scoreDate,
      ...
      const groupedByIp = new Map<
        string,
        Array<(typeof workScorePayloads)[number]>
      >();
      workScorePayloads
      ...
      const currentIpScores = ipRows.map((ip) => {
        ...
        const compositeScore =
          rows.length > 0
            ? rows.reduce(
                (sum, row) => sum + numberValue(row.compositeScore),
                0,
              ) / rows.length
            : 0;
        const momentumScore =
          rows.length > 0
            ? rows.reduce(
                (sum, row) => sum + numberValue(row.momentumScore),
                0,
              ) / rows.length
            : 0;
        const confidenceScore =
          rows.length > 0
            ? rows.reduce(
                (sum, row) => sum + numberValue(row.confidenceScore),
                0,
              ) / rows.length
            : 0;
        return {
      ...
      const rankedIps = [...currentIpScores].sort(
        (left, right) => right.compositeScore - left.compositeScore,
      );
...
export async function listLeaderboardRows(
  ...
  return withAnalyticsQuery([], async () => {
    ...
    const workIds = workRows.map((row) => row.id);
    const latestScoreDate = await getLatestWorkScoreDate(
      ctx,
      workIds,
      input.window,
    );
    ...
    const selectedScores = workIds.length
      ? await ctx.db
          ...
          .where(
            and(
              inArray(workScores.workId, workIds),
              eq(workScores.timeWindow, input.window),
              eq(workScores.scoreDate, latestScoreDate),
            ),
          )
      : [];
    const normalizedRows = workIds.length
    ...
    const providerIds = [
      ...new Set(normalizedRows.map((row) => row.sourceProviderId)),
    ];
    const providerRows = providerIds.length
      ? await ctx.db
          .select()
    ...
    let rows = selectedScores
      .map<LeaderboardRow | null>((score) => {
        ...
        return {
          ...
          ipName: work.franchiseId
            ? (ipById.get(work.franchiseId)?.name ?? null)
            : null,
          ...
          provenanceBadge: selectBestProvenance(
            workToTags.get(work.id) ?? ["metadata"],
          ),
          latestScoreDate,
        };
      })
    ...
    if (input.query) {
      ...
      rows = rows.filter((row) =>
        canonicalizeTitle(`${row.title} ${row.ipName ?? ""}`).includes(
          normalizedQuery,
        ),
      );
    }
  ...
  return withAnalyticsQuery<WorkDetail | null>(null, async () => {
    ...
    const [scoreRows, componentRows, normalizedRows] = await Promise.all([
      ctx.db
        .select()
        .from(workScores)
        .where(eq(workScores.workId, input.workId)),
      ctx.db
        .select()
        .from(scoreComponents)
        .where(eq(scoreComponents.workId, input.workId)),
      ctx.db
        .select()
        .from(normalizedObservations)
        .where(eq(normalizedObservations.workId, input.workId)),
    ]);
    ...
    const sourceBreakdown = providerIds.map((providerId) => {
      const rows = normalizedRows.filter(
        (row) => row.sourceProviderId === providerId,
      );
      return {
        sourceProviderId: providerId,
        sourceProviderName:
          providerById.get(providerId)?.name ?? "Unknown source",
        sourceProviderSlug: providerById.get(providerId)?.slug ?? "unknown",
        observationCount: rows.length,
        latestObservedAt:
          toIsoString(
            sortByNewest(rows, (row) => row.observedAt)[0]?.observedAt,
          ) ?? null,
        provenanceTags: [
          ...new Set(
            rows.map((row) => row.provenanceTag as AnalyticsProvenanceTag),
          ),
        ],
      };
    });

    const latestScoreDate = sortByNewest(scoreRows, (row) => row.scoreDate)[0]
      ?.scoreDate;
    const componentBreakdown: ScoreComponentBreakdown[] = componentRows
      .filter((row) =>
        latestScoreDate
          ? formatScoreDate(row.scoreDate) === formatScoreDate(latestScoreDate)
          : true,
      )
      .map((row) => ({
    ...
    return {
      ...
      scoreHistory: sortByNewest(scoreRows, (row) => row.scoreDate).map(
        (row) => ({
...
export async function listIpLeaderboardRows(
  ...
  return withAnalyticsQuery([], async () => {
    ...
    const latestScoreDate = await getLatestIpScoreDate(
      ctx,
      ipRows.map((row) => row.id),
      input.window,
    );
    if (!latestScoreDate) {
      return [];
    ...
    const scoreRows = ipRows.length
      ? await ctx.db
          ...
          .where(
            and(
              inArray(
                ipScores.franchiseId,
                ipRows.map((row) => row.id),
              ),
              eq(ipScores.timeWindow, input.window),
              eq(ipScores.scoreDate, latestScoreDate),
            ),
          )
      : [];
    ...
    if (input.query) {
      const normalizedQuery = canonicalizeTitle(input.query);
      rows = rows.filter((row) =>
        canonicalizeTitle(row.name).includes(normalizedQuery),
      );
    }
...
export async function getIpDetail(
  ...
  return withAnalyticsQuery<IpDetail | null>(null, async () => {
    ...
    const workRows = await ctx.db
      ...
      .where(
        and(
          eq(works.organizationId, organizationId),
          eq(works.franchiseId, input.ipId),
        ),
      );
    const scoreRows = await ctx.db
      .select()
      .from(ipScores)
      .where(eq(ipScores.franchiseId, input.ipId));
      ...
      topWorks:
        leaderboardResponse.status === "ready"
          ? leaderboardResponse.data
              .filter((row) => row.ipId === input.ipId)
              .slice(0, 10)
          : [],
      sourceCoverage,
    };
  ...
  return withAnalyticsQuery([], async () => {
    ...
    return providerRows
      ...
      .map((provider) => {
        ...
        const providerRawRows = rawRows.filter((row) =>
          providerBatchIds.includes(row.importBatchId),
        );
        const providerRawIds = providerRawRows.map((row) => row.id);
        const providerNormalizedRows = normalizedRows.filter((row) =>
          providerRawIds.includes(row.rawObservationId),
        );
        const providerUnresolvedRows = unresolvedRows.filter(
          (row) =>
        ...
        const staleCutoff = new Date(
          Date.now() - sourceFreshnessHours * 60 * 60 * 1000,
        );
        ...
        return {
          ...
          lastObservedAt:
            toIsoString(
              sortByNewest(providerRawRows, (row) => row.observedAt)[0]
                ?.observedAt,
            ) ?? null,
          ...
          mappedWorkCount: new Set(
            externalRows
              .filter((row) => row.sourceProviderId === provider.id)
              .map((row) => row.workId),
          ).size,
  ...
  return withAnalyticsQuery(
    ...
    async () => {
      ...
      return {
        ...
        unresolvedFlagCount:
          qualityResponse.status === "ready" ? qualityResponse.data.length : 0,
        latestImportAt:
