...
export const subsidiariesRelations = relations(
  subsidiaries,
  ({ one, many }) => ({
    organization: one(organizations, {
...
export const creatorAgreementsRelations = relations(
  creatorAgreements,
  ({ one }) => ({
    organization: one(organizations, {
...
export const payoutPeriodsRelations = relations(
  payoutPeriods,
  ({ one, many }) => ({
    organization: one(organizations, {
...
export const payoutLedgerEntriesRelations = relations(
  payoutLedgerEntries,
  ({ one }) => ({
...
export const sourceFamilyEnum = pgEnum("source_family_enum", [
  "ranking",
  "reviews",
  "awards",
  "search",
...
export const accessTypeEnum = pgEnum("access_type_enum", [
  "csv",
  "api",
  "scrape",
  "manual",
]);
export const confidenceTierEnum = pgEnum("confidence_tier_enum", [
  "gold",
  "silver",
  "bronze",
  "community",
...
export const provenanceTagEnum = pgEnum("provenance_tag_enum", [
  "direct",
  "estimated",
  "engagement",
  "awards",
...
export const scopeTypeEnum = pgEnum("scope_type_enum", [
  "global",
  "category",
  "ip",
]);
...
export const workExternalIds = pgTable("work_external_ids", {
  ...
  sourceProviderId: uuid("source_provider_id")
    .notNull()
...
export const importBatches = pgTable("import_batches", {
  ...
  sourceProviderId: uuid("source_provider_id")
    .notNull()
    .references(() => sourceProviders.id, { onDelete: "restrict" }),
  importType: text("import_type").notNull().default("csv"),
  uploadedBy: uuid("uploaded_by").references(() => profiles.id, {
    onDelete: "set null",
  }),
  status: importStatusEnum("status").notNull().default("pending"),
  autoReviewStatus: text("auto_review_status").notNull().default("pending"),
  autoReviewSummary: jsonb("auto_review_summary")
    .$type<{
...
export const qualityFlags = pgTable("quality_flags", {
  id: uuid("id").primaryKey().defaultRandom(),
  rawObservationId: uuid("raw_observation_id").references(
    () => rawObservations.id,
    { onDelete: "cascade" },
  ),
...
export const scoreComponents = pgTable("score_components", {
  ...
  componentScore: numeric("component_score", {
    precision: 10,
    scale: 6,
  }).notNull(),
...
export const workScores = pgTable("work_scores", {
  ...
  scoreDate: date("score_date").notNull(),
  timeWindow: timeWindowEnum("time_window").notNull(),
  compositeScore: numeric("composite_score", { precision: 10, scale: 6 })
    .notNull()
    .default("0"),
...
export const importFileRows = pgTable(
  "import_file_rows",
  {
    id: uuid("id").primaryKey().defaultRandom(),
    importBatchId: uuid("import_batch_id")
  ...
  (table) => ({
    importBatchRowUnique: uniqueIndex("import_file_rows_batch_row_unique").on(
      table.importBatchId,
      table.rowNumber,
    ),
  }),
...
export const sourceRecords = pgTable("source_records", {
  ...
  importFileRowId: uuid("import_file_row_id").references(
    () => importFileRows.id,
    { onDelete: "set null" },
  ),
...
export const sourceRecordMatches = pgTable(
  "source_record_matches",
  {
    ...
    selectedBy: uuid("selected_by").references(() => profiles.id, {
      onDelete: "set null",
    }),
  ...
  (table) => ({
    sourceWorkUnique: uniqueIndex(
      "source_record_matches_source_work_unique",
    ).on(table.sourceRecordId, table.workId),
    sourceIdx: index("idx_source_record_matches_source").on(
...
export const workAggregateSummaries = pgTable("work_aggregate_summaries", {
  ...
  aggregateDisplayRating: numeric("aggregate_display_rating", {
    precision: 6,
    scale: 3,
  }),
...
export const workAggregateSummaryHistory = pgTable(
  "work_aggregate_summary_history",
  {
    id: uuid("id").primaryKey().defaultRandom(),
    workId: uuid("work_id")
      .notNull()
...
export const franchisesRelations = relations(franchises, ({ one, many }) => ({
  organization: one(organizations, {
    fields: [franchises.organizationId],
    references: [organizations.id],
  }),
  works: many(works),
...
export const worksRelations = relations(works, ({ one, many }) => ({
  organization: one(organizations, {
    fields: [works.organizationId],
    references: [organizations.id],
  }),
  franchise: one(franchises, {
    fields: [works.franchiseId],
...
export const sourceProvidersRelations = relations(
  sourceProviders,
  ({ many }) => ({
    externalIds: many(workExternalIds),
...
export const workExternalIdsRelations = relations(
  workExternalIds,
  ({ one }) => ({
    work: one(works, {
      fields: [workExternalIds.workId],
      references: [works.id],
    }),
    sourceProvider: one(sourceProviders, {
      fields: [workExternalIds.sourceProviderId],
...
export const importBatchesRelations = relations(
  ...
  ({ one, many }) => ({
    ...
    sourceProvider: one(sourceProviders, {
      fields: [importBatches.sourceProviderId],
      references: [sourceProviders.id],
    }),
...
export const rawObservationsRelations = relations(
  rawObservations,
  ({ one, many }) => ({
    importBatch: one(importBatches, {
      fields: [rawObservations.importBatchId],
      references: [importBatches.id],
    }),
    sourceProvider: one(sourceProviders, {
      fields: [rawObservations.sourceProviderId],
      references: [sourceProviders.id],
    }),
...
export const normalizedObservationsRelations = relations(
  normalizedObservations,
  ({ one }) => ({
    rawObservation: one(rawObservations, {
      fields: [normalizedObservations.rawObservationId],
      references: [rawObservations.id],
    }),
    work: one(works, {
      fields: [normalizedObservations.workId],
      references: [works.id],
    }),
    sourceProvider: one(sourceProviders, {
      fields: [normalizedObservations.sourceProviderId],
      references: [sourceProviders.id],
    }),
  }),
);
...
export const qualityFlagsRelations = relations(qualityFlags, ({ one }) => ({
  rawObservation: one(rawObservations, {
    fields: [qualityFlags.rawObservationId],
    references: [rawObservations.id],
  }),
  work: one(works, { fields: [qualityFlags.workId], references: [works.id] }),
  resolvedBy: one(profiles, {
    fields: [qualityFlags.resolvedBy],
    references: [profiles.id],
  }),
}));

export const scoreComponentsRelations = relations(
  scoreComponents,
  ({ one }) => ({
    work: one(works, {
      fields: [scoreComponents.workId],
      references: [works.id],
    }),
  }),
);
...
export const importFileRowsRelations = relations(importFileRows, ({ one }) => ({
  importBatch: one(importBatches, {
    fields: [importFileRows.importBatchId],
    references: [importBatches.id],
  }),
}));

export const sourceRecordsRelations = relations(
  sourceRecords,
  ({ one, many }) => ({
    organization: one(organizations, {
      fields: [sourceRecords.organizationId],
      references: [organizations.id],
    }),
    sourceProvider: one(sourceProviders, {
      fields: [sourceRecords.sourceProviderId],
      references: [sourceProviders.id],
    }),
    importBatch: one(importBatches, {
      fields: [sourceRecords.importBatchId],
      references: [importBatches.id],
    }),
    importFileRow: one(importFileRows, {
      fields: [sourceRecords.importFileRowId],
      references: [importFileRows.id],
    }),
...
export const sourceRecordMatchesRelations = relations(
  sourceRecordMatches,
  ({ one }) => ({
    sourceRecord: one(sourceRecords, {
      fields: [sourceRecordMatches.sourceRecordId],
      references: [sourceRecords.id],
    }),
    work: one(works, {
      fields: [sourceRecordMatches.workId],
      references: [works.id],
    }),
    selectedByProfile: one(profiles, {
      fields: [sourceRecordMatches.selectedBy],
      references: [profiles.id],
    }),
  }),
);

export const workSourceSummariesRelations = relations(
  workSourceSummaries,
  ({ one }) => ({
    work: one(works, {
      fields: [workSourceSummaries.workId],
      references: [works.id],
    }),
    sourceProvider: one(sourceProviders, {
      fields: [workSourceSummaries.sourceProviderId],
      references: [sourceProviders.id],
    }),
    sourceRecord: one(sourceRecords, {
      fields: [workSourceSummaries.sourceRecordId],
      references: [sourceRecords.id],
    }),
  }),
);

export const workAggregateSummariesRelations = relations(
  workAggregateSummaries,
  ({ one }) => ({
    work: one(works, {
      fields: [workAggregateSummaries.workId],
      references: [works.id],
    }),
    organization: one(organizations, {
      fields: [workAggregateSummaries.organizationId],
      references: [organizations.id],
    }),
  }),
);

export const workAggregateSummaryHistoryRelations = relations(
  workAggregateSummaryHistory,
  ({ one }) => ({
    work: one(works, {
      fields: [workAggregateSummaryHistory.workId],
      references: [works.id],
    }),
  }),
);
